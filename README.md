...
| PostgreSQL | 18.4 | [当前源码](src/lib.rs)仍有精确 18.4 runtime gate，其他 PG18 minor 会被拒绝 |
...
1. 停止整个候选 PostgreSQL 实例，确保没有 backend 已加载旧 `.so`。
2. 验证目标 `pg_config` 报告 PostgreSQL 18.4，并读取真实 `pkglibdir`。
3. 把同一候选构建生成的 `libpggomtm.so` 安装为 `pggomtm.so`。
4. 在 `postgresql.conf` 中配置 validator library。
5. 启动或重建全部 backend，再运行候选加载与拒绝路径验证。
...
install -m 0755 ./libpggomtm.so "$pkglibdir/pggomtm.so"
```
...
3. 停止或重建所有受影响的 PostgreSQL backend，再切换镜像或 `.so`。
4. 重新运行 loader、拒绝路径和 ABI 门禁。

回退时选择上一份已验证候选，重复停止、切换、启动与验证步骤。不要热覆盖 `.so`，不要现场编译，也不要恢复第二份源码或认证 fallback。
