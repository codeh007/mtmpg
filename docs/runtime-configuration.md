...
当前仓库已经实现固定路径读取、权限与schema校验、同一真实父目录与设备号检查，以及每个新backend在startup建立独立不可变snapshot并在shutdown释放。轮换期间既有backend保持原snapshot，后续backend读取原子替换后的完整active与retiring key集合；缺失、损坏或非原子布局都会fail closed，且没有旧cache或内置key回退。
