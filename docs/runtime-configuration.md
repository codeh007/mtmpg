...
当前仓库已经实现本契约的固定路径文件读取、权限与schema校验，以及每个新backend在startup建立独立不可变snapshot并在shutdown释放。缺失或损坏材料会使startup fail closed，且没有旧cache或内置key回退。正式validate callback尚未消费该snapshot，无gate artifact继续返回`authorized=false`，不能作为可部署validator；原子替换轮换证明和正式callback接入仍属于后续门禁。
