...
- Database token矩阵已经覆盖OAuth client与API-key credential actor、三种profile、authority version、ID/time边界、algorithm、audience/scope与tampered signature；closed role、配置扩权与service/migration/cluster/unknown role拒绝门禁已经取得[精确远端commit的Native CI证据](evidence/issue-116/native-ci-bootstrap.md)。Identity allocator和失败reason仍需完成独立门禁。
