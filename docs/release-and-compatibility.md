...
Database token contract 1固定ES256、唯一issuer/audience、`database` scope、30至300秒TTL、deny-unknown claims、actor二选一以及closed profile-role：
...
`.github/workflows/ci.yml`同时服务Pull Request、`main` push和release调用。PR与main只运行只读门禁：

1. 生成一次Cargo.lock并解析Rust、PG18和builder/runtime digest。
2. 运行Rust领域、C/Rust ABI、真实PG18 backend和final-image门禁。
3. 只构建一次production OCI archive。
4. 没有SemVer tag时不写GHCR、Release或attestation。

`.github/workflows/release.yml`只由 `v<semver>` tag进入。去除前导 `v` 后的version必须与Cargo package version相等。Release调用同一CI并上传本次run已验证的archive；最小写权限publish job只下载和推送该archive，不运行Cargo、不重新解析依赖，也不执行第二次Docker build。
...
- Stable例如 `v0.1.0`，从自己的tag重新解析、完整测试、构建和发布；全部成功后才把 `latest`更新为该stable digest。
...
消费者使用明确的 `ghcr.io/codeh007/mtmpg:<semver>`，不得现场编译module、复制native测试矩阵或增加本地image fallback。Gomtmui的TLS、sub2api、pgAdmin、ACL/RLS、OAuth issuer和SQL executor由gomtmui自身领域change验证，不阻塞mtmpg release。
