...
- Database token矩阵已经覆盖OAuth client与API-key credential actor、三种profile、authority version、ID/time边界、algorithm、audience/scope与tampered signature；closed role与forbidden-role门禁已经取得[远端CI证据](evidence/issue-116/native-ci-bootstrap.md)，allocator、`system_user`与decoded identity往返也已取得[真实libpq证据](evidence/issue-116/production-identity-roundtrip.md)。失败reason仍需完成独立门禁。
