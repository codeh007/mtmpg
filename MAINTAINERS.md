...
只有合法validator `v<semver>`或executor `executor-v<semver>` tag可以进入对应release workflow。Tag version必须与目标Cargo package version一致；publish job只能推送同一run只读CI已验证的OCI archive，不得checkout源码、运行Cargo、重新解析依赖或执行Docker build。
