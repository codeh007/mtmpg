...
- mtmpg SemVer、release manifest schema和release权限变化
- 新平台、架构、libc或runtime发行版

## SemVer release

只有合法SemVer tag可以进入release workflow。Tag version必须与Cargo package version一致；publish job只能推送同一run只读CI已验证的OCI archive，不得checkout源码、运行Cargo、重新解析依赖或执行Docker build。

Prerelease与stable分别执行完整门禁并保存自己的Cargo.lock、resolved inputs、module、OCI digest、SBOM、provenance和attestation。Prerelease不得更新 `latest`；stable全部成功后才更新。任何既有version、tag、asset或Release都不得覆盖。
