...
- 本地 MAY运行相关 locked Rust test、C probe、SQL runtime probe、`cargo fmt --check`与Clippy以快速定位问题，但这些结果不能完成OpenSpec task、consumer gate或发布门禁。
- 一次性 bootstrap 期间，把精确、可审查的 commit 非 force push到批准的远端功能 ref；进入受保护 `main` 后，为 Issue 创建短期分支和 PR，由 `.github/workflows/native-ci.yml` 运行根 `Dockerfile` 定义的唯一 build graph。
- 原生ABI、pgrx、PostgreSQL minor、artifact或门禁变化必须取得对应远端commit的成功`Native CI` run；不得用本地image、tag或终端日志替代。
- 交付前通过后续cold authority mode运行无缓存完整Docker门禁、secret扫描，并对本地待提交内容运行`git diff --check`。
...
公开的 `main` 是 development branch，不是 stable 标记。当前 `issue-116-extract-pggomtm` 只用于一次性 bootstrap；完成追溯审计、cold build、whole-branch review 和 source identity 核对后，按 OpenSpec 非 force fast-forward 到 `main`，不得同时创建 tag、Release 或 `latest`。

稳态普通变更由 Agent 管理拉取请求（Pull Request，PR）生命周期：从受保护 `main` 创建 Issue 范围内短期分支，创建或更新 PR，等待 required `Native CI`，处理失败与讨论，然后启用 squash auto-merge。合并后由 GitHub 删除源分支，用户不负责手工维护 PR。Auto-merge 和 ruleset 尚未启用时，Agent 不得用直接合并冒充该流程。

Required approving review 数为 `0`，但高风险变化仍需显式技术审查。pgrx、JOSE、Rust toolchain、PostgreSQL minor、官方 base/header、Actions source/pin、release workflow 或写权限缺少上游 diff、风险和验证证据时，不得启用 auto-merge。

公开 fork PR 只能使用临时 runner、read-only token 和无 secret 上下文，禁止 `pull_request_target`、package/Release/attestation 写权限。后续 `mtmpg-postgres` package 公开读取，写入只属于受保护 `main` 的 trusted job；消费者必须固定完整 OCI digest，不得保存 private pull credential。
