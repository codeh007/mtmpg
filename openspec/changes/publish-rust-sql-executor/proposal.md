...
- 新增独立、可消费的`ghcr.io/codeh007/mtmpg-executor:0.1.3`非root image与版本化release入口；标准GitHub CI统一解析Rust/PG18/libpq输入，运行Rust、并发、真实PG18、TLS、OAuth、取消和final-image门禁并生成SBOM、provenance与attestation。历史`executor-v0.1.0`保留为不可变但无附件的失败Release，`executor-v0.1.1`保留为已推送image、attestation及完整draft附件但发布作业失败的历史，`executor-v0.1.2`保留为已推送image、attestation及完整draft附件但受Release列表最终一致性影响而失败的历史；三个身份均绝不覆盖、移动或重新发布。
...
- 交付：executor image定义、CI/release workflow、manifest、SBOM、provenance、attestation、README、AGENTS与维护文档；0.1.3的发布必须先在draft Release上传全部附件，以有界重试等待Release列表可见并唯一解析draft数值ID，再发布并冻结Release。
