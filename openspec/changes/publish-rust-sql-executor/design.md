...
根Cargo package继续命名`pggomtm`并保持validator版本线；新增`executor/` package，历史初始版本为`0.1.0`，当前前向修复版本为`0.1.3`。Workspace只包含这两个产品package并共享一次解析的`Cargo.lock`。Validator中与pgrx无关的profile、role、claims schema和identity输入约束抽成纯Rust contract模块；executor以关闭validator默认feature的path dependency复用该模块，不能复制claim struct、profile闭集或role映射。
...
现有`v<semver>` tag、`ghcr.io/codeh007/mtmpg:<semver>`和`latest`只属于validator，`v0.2.0`不移动、不重建。Executor使用`executor-v<semver>` annotated tag、`ghcr.io/codeh007/mtmpg-executor:<semver>`和独立GitHub Release。`executor-v0.1.0`已经是不可变但无附件的失败历史；`executor-v0.1.1`已推送image与attestation并建立完整draft附件，但workflow错误地通过tag endpoint查询draft而失败；`executor-v0.1.2`也已推送image与attestation并建立完整draft附件，但workflow在GitHub Release列表完成最终一致性前只查询一次而失败。修复后的当前稳定消费目标是`executor-v0.1.3`/`0.1.3`。Executor stable MAY维护自己的`latest`，但gomtmui只消费明确SemVer。

0.1.3发布必须使用精确的main GREEN SHA。只读CI先验证tag、package version、共享lockfile、PG18/libpq输入和final image，再在draft Release中上传manifest、checksums、lockfile、resolved inputs、SBOM、provenance与attestation；workflow必须对Release列表进行最多12次、间隔5秒的有界查询，0个匹配时等待、1个匹配时唯一解析draft数值ID、多个匹配时立即失败，且不能使用只返回published Release的tag endpoint。全部附件核验通过后才将同一个Release发布并冻结。已形成的tag、versioned image、attestation或Release历史不得覆盖，发布错误必须通过递增patch前向修复。
...
5. 保留`executor-v0.1.0`、`executor-v0.1.1`与`executor-v0.1.2`失败历史，在既有draft发布顺序与draft ID解析基础上补充Release列表最终一致性的有界轮询，并将executor package递增到`0.1.3`。
6. 在精确main GREEN SHA创建annotated `executor-v0.1.3` tag，由标准workflow一次发布`0.1.3` image、Release和供应链材料；匿名核对tag、source、digest、SBOM、provenance与attestation，并核对validator `v0.2.0`身份未变化。
