...
根Cargo package继续命名`pggomtm`并保持validator版本线；新增`executor/` package，初始版本`0.1.0`。Workspace只包含这两个产品package并共享一次解析的`Cargo.lock`。Validator中与pgrx无关的profile、role、claims schema和identity输入约束抽成纯Rust contract模块；executor以关闭validator默认feature的path dependency复用该模块，不能复制claim struct、profile闭集或role映射。
...
现有`v<semver>` tag、`ghcr.io/codeh007/mtmpg:<semver>`和`latest`只属于validator，`v0.2.0`不移动、不重建。Executor使用`executor-v<semver>` annotated tag、`ghcr.io/codeh007/mtmpg-executor:<semver>`和独立GitHub Release；初始stable为`executor-v0.1.0`/`0.1.0`。Executor stable MAY维护自己的`latest`，但gomtmui只消费明确SemVer。
...
5. 创建annotated `executor-v0.1.0` tag，由标准workflow一次发布image、Release和供应链材料；匿名核对tag、source、digest、SBOM、provenance与attestation。
6. Gomtmui只在resolved identity与全部platform gate通过后选择该executor image；失败时保持SQL tool未注册。
