...
`pggomtm` 当前是 `/workspace/gomtmui/native/pggomtm/` 下约 124 KB 的未跟踪 Rust 源码、测试与构建定义，本地 `target/` 约 2 GB且不属于可迁移内容。原型已经证明 PostgreSQL 18.4 OAuth callback ABI、PostgreSQL allocator/panic 边界、ES256/JWKS、严格 claims 和版本化 identity 可行，但正常 release feature 仍默认拒绝 token；只有测试 feature 使用内置 gate JWKS。`/workspace/mtmpg` 已连接私有远程 `codeh007/mtmpg`，目前仅有初始提交和 Cargo `.gitignore`。
...
新增最小build-time binding步骤，从目标`pg_config --includedir-server/libpq/oauth.h`读取官方header，只allowlist OAuth magic、三种ABI struct和callback类型，输出到Cargo `OUT_DIR`。构建同时记录header digest，并保留用官方C compiler执行的size/offset/layout probe。Rust源码不再手写这些struct、字段布局或magic常量。
...
正式artifact不编译`abi-gate`、`abi-runtime-gate`或`pgx-oauth-gate`能力。每个新OAuth backend在validator startup读取版本化只读config和public JWKS snapshot，完成文件权限、大小、schema、issuer、audience、key数量与key类型检查；缺失、损坏或不匹配立即fail closed。平台通过同文件系统内原子替换发布JWKS/config，后续新backend读取新snapshot；第一阶段不增加signal handler、网络fetch或跨backend shared cache。
...
仓库当前为private且套餐拒绝branch protection/rulesets。change不擅自改变visibility或付费套餐；README/SECURITY必须记录这一治理限制，并采用批准的merge策略、合并后删分支、发布tag人工复核和CI证据弥补，但不得声称已具备无法启用的强制保护。普通后续PR可优先squash；首个跨仓库验收分支必须以fast-forward把已验证branch HEAD原样推进到`main`，确保stable source commit与candidate证据一致。

### 9. Prerelease先于stable，禁止把fail-closed原型冒充可安装版本

源码迁移和CI建立后，先把`issue-116-extract-pggomtm`功能分支推送到远程，再从该明确commit发布`v0.1.0-alpha.1`或short-SHA image用于跨仓库门禁；prerelease不得更新`latest`。只有正式callback已经消费外部只读JWKS/config、无gate feature、真实OAuth allow/deny矩阵和gomtmui candidate集成全部通过后，才能以fast-forward把同一通过的branch HEAD推进到`main`，再为已经验证的同一OCI digest创建首个stable release与`latest`别名。
...
3. 建立PR CI、release workflow、SBOM/provenance、GHCR和immutable Release；收紧可用GitHub Actions设置，并记录当前private套餐无法启用的治理项。提交并推送`issue-116-extract-pggomtm`功能分支，使远程CI和prerelease有可审计source commit。
4. 从该功能分支commit发布prerelease/short-SHA image和manifest，使用真实PG18 candidate完成loader、OAuth、role、identity与负向矩阵；不得更新`latest`、发布stable或生产说明。
