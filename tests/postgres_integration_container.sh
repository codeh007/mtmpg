...
run_production_backend_smoke() {
  ...
  psql_command \
    'CREATE ROLE ordinary LOGIN; CREATE ROLE business_admin LOGIN'
  generate_fixtures "${fixture_root}"
  "${ARTIFACT_ROOT}/pggomtm_oauth_smoke_client" \
  ...
  expect_rejected "${fixture_root}" oauth-legacy-delegation ordinary
  expect_rejected "${fixture_root}" oauth-legacy-role ordinary
  expect_rejected "${fixture_root}" oauth-legacy-profile ordinary
